...
const TokensRoute: Route = async (fastify, ctx) => {
	fastify.get('/tokens', async (req, resp) => {
		...
		return resp.view('tokens/index.ejs', {
			...
			tokens: ctx.tokens.getTokens().map(token => ({
				id: token.tokenId,
				description: token.slot.getToken().label,
	...
	fastify.get<{ Params: IdParamsType }>(
		...
		async (req, resp) => {
			...
			return token.withSession(session => {
				const objs = Array.from(session.find()).map(obj =>
					obj.toType()
				);
				...
				return resp.view('tokens/details.ejs', {
					...
					token: {
						...
						slot: {
							...
							isHardware:
								token.slot.flags & pkcs11.SlotFlag.HW_SLOT,
							isRemovable:
								token.slot.flags &
								pkcs11.SlotFlag.REMOVABLE_DEVICE,
							isInitialized:
								token.slot.flags &
								pkcs11.SlotFlag.TOKEN_PRESENT,
